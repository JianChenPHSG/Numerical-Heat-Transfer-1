...
**************************   revise  **********************************************
	...
	CALL SETUP0 
      !init all the relax factor
      CALL GRID
      CALL START
      ! give specefic relax factor(1,2,4,11)
	C_TIME=0
	E_SUM=1
	E=1
	!REV_P=REV_P+0.1
...
*--------------------------------------------------------------------------
      SUBROUTINE SOLVE
****************************************************************************
	PARAMETER (NX=1152,NY=1152,NXY=1152)
...
**************************   revise  **********************************************
	...
	E=(SSUM**(1/2.0))-(ESUM**(1/2.0))
      !DSUM=(SSUM**(1/2.0))/(ESUM**(1/2.0))  
      ESUM=SSUM
      !E=DSUM-1
	DE=E-E_1

      CALL MODREX
!	IF(0.9<ESUM<1.1) THEN
!	  REV_P=1
!	ELSEIF(ESUM<0.9) THEN
!	  REV_P=1.2
!	 ELSE
!	  REV_P=0.6
!	 END IF 
...
*---COMEE HERE TO CORRECT THE PRESSURE AND VELOCITIES
      DO 501 J=2,M2
      DO 501 I=2,L2
      ! add an switch to choose
      !
      !then add an rule to accelerate the method

      IF(.TRUE.)  RELAX(NP)=REV_P
      P(I,J)=P(I,J)+PC(I,J)*RELAX(NP)
      !only revise every node's pressure by relax
